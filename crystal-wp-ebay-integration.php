<?php
/**
 * Plugin Name: crystal wp ebay integration
 * Plugin URI: http://wordpress.org/plugins/crystaltechifyconnect/
 ...
 * Version: 1.0
 * Text Domain: crystaltechify-linq
 * WC requires at least: 2.0.0
 * WC tested up to: 6.3.1
 * License: GPLv2
 * License URI: http://www.gnu.org/licenses/gpl-2.0.html
 *
 * @package crystaltechify LINQ by crystaltechify
 * @Version: 1.0
 */
...
define( 'crystaltechifyCONNECT_VERSION', '1.0' );
define( 'crystaltechifyCONNECT_RESELLERKEY', '' );

if ( ! class_exists( 'crystaltechifyConnect' ) ) :

final class crystaltechifyConnect {
	...
	*/
	public function query_vars( $vars ) {
		...
		$vars[] = 'crystaltechify';
		$vars[] = 'crystaltechify-proxy-route';
		$vars[] = 'crystaltechify-sync-route';
		return $vars;
	}
	...
	*/
	public function nocache_headers( $headers ) {
		...
		if ( isset( $_GET['page'] ) &&
			substr( $_GET['page'], 0, 7 ) === 'crystaltechify' &&
			$_GET['page'] !== 'crystaltechify-templates' ) {
			$headers = array(
	...
	*/
	private function check_hash() {
		...
		if ( ! isset( $_SERVER['HTTP_X_crystaltechifyNONCE'] ) ||
			! isset( $_SERVER['HTTP_X_crystaltechifyKEY'] ) ) {
		...
		$r = get_option( 'crystaltechify_key' ) . $_SERVER['HTTP_X_crystaltechifyNONCE'];
		$base = hash( 'sha256', $r, true );
		$checkHash = base64_encode( $base );
		if ( ! hash_equals( $_SERVER['HTTP_X_crystaltechifyKEY'], $checkHash ) ) {
			$this->sendHttpHeaders(
				'400 Security Error',
				array(
					'Content-Type' => 'application/json',
	...
	*/
	...
	public function inhibit_order_emails( $enabled, $order ) {
		...
		if($enabled && $order) {
			...
			if( get_post_meta( $orderId, '_crystaltechify_orderid' ) ) {
	...
	*/
	private function json_encode( $arg ) {
		if ( function_exists( 'wp_json_encode') ) {
			...
			throw new Exception( __( 'PHP missing json library - please upgrade php or wordpress', 'crystaltechify-linq' ) );
		}
	}
	...
	*/
	private function get_product( $id ) {
		if ( function_exists( 'wc_get_product') ) {
			...
			throw new Exception( __( 'WooCommerce wc_get_product function is missing - please reinstall or activate WooCommerce', 'crystaltechify-linq' ) );
		}
	}
	...
	*/
	public function sync() {
		...
		$type = $wp->query_vars['crystaltechify-sync-route'];
		if ( strtolower( $_SERVER['REQUEST_METHOD'] ) == 'get' ) {
			if ( $type == 'test' ||
				...
				$response = array(
					...
					'version' => crystaltechifyCONNECT_VERSION
				);
				...
				$orders = $wpdb->get_results(
					$wpdb->prepare(
						"SELECT (".
							"SELECT meta_value FROM `{$wpdbsiteprefix}postmeta` WHERE post_id = P.id AND meta_key = '_crystaltechify_orderid' AND ".
								"(".
									"EXISTS ( SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND meta_value = %d AND post_id = P.id ) ".
									"OR NOT EXISTS ( SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND post_id = P.id ) ".
								")".
							") AS id, ".
						...
						" AND ID IN (".
							"SELECT post_id FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_orderid' AND (".
								"EXISTS ( SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND meta_value = %d AND post_id = P.id ) ".
								"OR NOT EXISTS ( SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND post_id = P.id ) ".
							")".
						") ORDER BY ID LIMIT %d, %d",
				...
				if ( $page == 0 ) {
					$total_count = $wpdb->get_var(
						$wpdb->prepare(
							"SELECT COUNT(*) FROM `{$wpdbsiteprefix}posts` AS P WHERE post_type = 'shop_order' AND post_date > DATE_SUB( CURRENT_TIMESTAMP(), INTERVAL 90 DAY ) AND ID IN ( SELECT post_id FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_orderid' AND ( EXISTS ( SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND meta_value = %d AND post_id = P.id ) OR NOT EXISTS (SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND post_id = P.id )))",
							$merchantid
						)
					);
				}
						...
						if ( sizeof( $filestozip ) == 0 ) {
							...
							if ($zipsuccess) {
								$headers = array(
									...
									'Pragma' => 'no-cache',
									'Expires' => 'Thu, 01 Jan 1970 00:00:00 GMT',
									'X-crystaltechify-Content-Type' => 'application/zip',
								...
								$this->sendHttpHeaders(
									...
									array(
										'Content-Type' => 'application/json',
										'Cache-Control' => 'no-cache, no-store',
										'X-crystaltechify-Content-Type' => 'application/json',
										'Expires' => 'Thu, 01 Jan 1970 00:00:00 GMT',
										'Pragma' => 'no-cache'
									)
								);
				...
				$siteverification = get_option( 'crystaltechify_site_verification' );
				...
				$conversiontracking = get_option( 'crystaltechify_conversion_tracking' );
			...
			if ( $type === 'createorder' ) {
				...
				try {
					...
					$ordercontent = $xml->entry->content->children( 'http://api.crystaltechify.com/schemas/2009/' );
					...
					if ( isset( $ordercontent->wooneworderpush )
						...
						if(!empty( $ordercontent->orderid )
							...
							"WHERE post_id = %d AND (meta_key = '_crystaltechify_merchantid' AND meta_value = %d) " .
							"LIMIT 1";
						...
						if(!$order_id) {
							...
							"(PM2.post_id = PM.post_id AND PM2.meta_key = '_crystaltechify_merchantid' AND PM2.meta_value = %d) " .
							"WHERE (PM.meta_key = '_crystaltechify_orderid' AND PM.meta_value = %d) " .
							"LIMIT 1";
						...
						" WHERE meta_key = '_crystaltechify_orderid' AND meta_value = %d AND post_id = P.ID ) " .
						" AND (".
							" EXISTS (SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND meta_value = %d AND post_id = P.ID)" .
							" OR NOT EXISTS (SELECT 1 FROM `{$wpdbsiteprefix}postmeta` WHERE meta_key = '_crystaltechify_merchantid' AND post_id = P.ID)"
						.")" .
					...
					if ( ! $order_id ) {
						...
						update_post_meta( $order_id, '_crystaltechify_orderid', (int)$ordercontent->orderid );
						update_post_meta( $order_id, '_crystaltechify_merchantid', (int)$ordercontent->merchantid );

						if ( $amazonorderid ) {
							update_post_meta( $order_id, '_crystaltechify_amazonorderid', $amazonorderid );
						}
						if ( $amazonfulfillmentchannel ) {
							update_post_meta( $order_id, '_crystaltechify_amazonfulfillmentchannel', $amazonfulfillmentchannel );
						}

						if ( $ebayusername ) {
							update_post_meta( $order_id, '_crystaltechify_ebayusername', $ebayusername );
						}

						if ( $ebaysalesrecordnumber ) {
							update_post_meta( $order_id, '_crystaltechify_ebaysalesrecordnumber', $ebaysalesrecordnumber );
						}

						if ( $ebaytransactionid ) {
							update_post_meta( $order_id, '_crystaltechify_ebaytransactionid', $ebaytransactionid );
						}
				...
				update_option( 'crystaltechify_site_verification' , file_get_contents( 'php://input' ) );
				...
				$conversiontracking = intval( get_option( 'crystaltechify_conversion_tracking' ) ) + 1;

				update_option( 'crystaltechify_conversion_tracking' , strval( $conversiontracking ) );

				$upload_dir = wp_upload_dir();
				$conversion_tracking_file = '/crystaltechify/conversion-tracking.js';
	...
	*/
	public function order_is_editable( $editable, $order ) {
		$crystaltechify_order_id = get_post_meta( $order->get_id(), '_crystaltechify_orderid', true);
		if ( is_numeric( $crystaltechify_order_id ) && $crystaltechify_order_id !== 0 ) {
			return false;
		}
	...
	*/
	public function order_buttons( $order ) {
		$crystaltechify_order_id = get_post_meta( $order->get_id(), '_crystaltechify_orderid', true );
		if ( is_numeric( $crystaltechify_order_id ) && $crystaltechify_order_id !== 0 ) {
			$ebay_user = get_post_meta( $order->get_id(), '_crystaltechify_ebayusername', true );
			$merchantid = get_post_meta( $order->get_id(), '_crystaltechify_merchantid', true );
			if ( $ebay_user ) {
				?>
				<p class="form-field form-field-wide crystaltechify-order-buttons">
				<a href="<?php echo htmlspecialchars( admin_url( 'crystaltechify/ebaysale?orderid='.$crystaltechify_order_id.'&merchantid='.$merchantid ) ) ?>" target="crystaltechify!sale" class="button"><?php esc_html_e( 'eBay Order', 'crystaltechify-linq' ) ?> &rarr;</a>
				<a href="<?php echo htmlspecialchars( admin_url( 'crystaltechify/ebayuser?orderid='.$crystaltechify_order_id.'&merchantid='.$merchantid) ) ?>" target="crystaltechify!user" class="button"><?php esc_html_e( 'eBay User', 'crystaltechify-linq' ) ?><?php echo $ebay_user ? ' : '.htmlspecialchars( $ebay_user ) : ''; ?> &rarr;</a>
				</p>
				<?php
			}
			$amazon_order = get_post_meta( $order->get_id(), '_crystaltechify_amazonorderid', true );
			if ( $amazon_order ) {
				?>
				<p class="form-field form-field-wide crystaltechify-order-buttons">
				<a href="<?php echo htmlspecialchars( admin_url( 'crystaltechify/amazonsale?orderid='.$crystaltechify_order_id.'&merchantid='.$merchantid ) ) ?>" target="crystaltechify!sale" class="button"><?php esc_html_e( 'Amazon Order', 'crystaltechify-linq' ) ?> &rarr;</a>
				</p>
				<?php
			}
		}
	...
	* requests for UI and proxies those requests from the server back to crystaltechify
	*
	*/
	public function proxy() {
		...
		if ( isset( $_GET['productid'] ) ) {
			wp_redirect( admin_url( 'post.php?post='.urlencode( wp_unslash( $_GET['productid'] ) ).'&action=edit#crystaltechify_product_data' ) );
			exit;
		}

		$HostKey = get_option( 'crystaltechify_key' );
		...
		$path = $wp->query_vars['crystaltechify-proxy-route'] . ( preg_match( '/\/(?:\\?|$)/', $_SERVER['REQUEST_URI'] ) ? '/' : '' );

		$storeId = '0';
		$merchantid = get_option( 'crystaltechify_merchantid' );
		...
		$remoteUrl = 'https://ui.crystaltechify.com/' . $merchantid . '/'. $path . ( $querystring ? '?'.$querystring : '' );

		$adminUrl = admin_url( 'crystaltechify/ebaytab/'.$storeId.'/'.$merchantid.'/' );

		$requestHeaders = array(
							'X-crystaltechify-Cart' => 'woocommerce',
							'X-crystaltechify-Version' => crystaltechifyCONNECT_VERSION,
							'X-HostKey' => $HostKey,
		...
		if ( is_multisite() ) {
			$certPath = $upload_dir['basedir'].'/sites/'.get_current_blog_id().'/crystaltechify.crt';
		} else {
			$certPath = $upload_dir['basedir'].'/crystaltechify.crt';
		}
		...
		for ( $retry = 0; ; $retry++ ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( $response->get_error_code() == 'http_request_failed' ) {
					$certResponse = wp_remote_get( 'http://ui.crystaltechify.com/crystaltechify.crt' );
	...
	*/
	public function parse() {
		...
		if ( ! empty( $wp->query_vars['crystaltechify'] ) &&
			in_array( $wp->query_vars['crystaltechify'], array( 'proxy','sync' ), true ) ) {
			$crystaltechifyMode = $wp->query_vars['crystaltechify'];

			if ( $crystaltechifyMode == 'sync' ) {
				$this->sync();
			} elseif ( $crystaltechifyMode == 'proxy' ) {
	...
	*/
	private function reseller_key() {
		return crystaltechifyCONNECT_RESELLERKEY;
	}

	/**
	* POST handler for create account on crystaltechify servers for this woocommerce instance
	*
	*/
	public function create_account() {
		...
		if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
			...
			check_admin_referer( 'crystaltechify-create' );
			...
			if ( $_POST['method'] == 'email' ) {
				...
				$httpOptions = array(
								...
								'body' => $this->json_encode(
									array (
										...
										'crystaltechifyversion' => crystaltechifyCONNECT_VERSION
									)
								)
							);

				$response = wp_remote_request( 'https://ui.crystaltechify.com/create', $httpOptions );
				...
				if ( $response ) {
					...
					curl_setopt_array(
						...
						array(
							...
							CURLOPT_URL => 'https://ui.crystaltechify.com/create',
							CURLOPT_POST => 1,
				...
				update_option( 'crystaltechify_merchantid' , 	$result['merchantid'] );
				update_option( 'crystaltechify_key',			$result['hostkey'] );

				wp_cache_flush();

				wp_redirect( 'admin.php?page=crystaltechify' );
				...
				wp_redirect(
					'https://ui.crystaltechify.com/register?finalurl='.
					urlencode( admin_url( 'admin-post.php?action=crystaltechify_create&_wpnonce='.urlencode( wp_create_nonce( 'crystaltechify-create' ) ) ) ).
					'&type=woocommerce'.
					...
					'&crystaltechifyversion='.urlencode( crystaltechifyCONNECT_VERSION )
				);
			}

		} else {

			if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'crystaltechify-create') ) {
				wp_die( '<p>'.esc_html__( 'URL Security Check has failed, please start the process again.', 'crystaltechify-linq' ).'</p>' );
			}
	...
	*/
	public function update_template() {
		...
		if ( !current_user_can( 'edit_themes' ) ) {
			wp_die( '<p>'.esc_html__( 'You do not have sufficient permissions to edit templates for this site.', 'crystaltechify-linq' ).'</p>' );
		}
		...
		wp_redirect( admin_url( 'admin.php?page=crystaltechify-templates&file='.urlencode( $filename ).( $updated ? '&updated=true' : '' ) ) );
		exit();
	}

	/**
	* common function used to render a proxied crystaltechify page that checks
	* for a valid registered crystaltechify account
	*
	...
	*/
	private function admin_tab( $url, $tabclass ) {
		...
		if ( ! is_numeric( $merchantid ) ) {
			...
			<iframe id="dummy-data" frameborder="0" src="https://crystaltechify.com/xpressgriddemo/ebayedit/"></iframe>
			<div id="dummy-data-overlay"></div>
			<div id="create-account-modal">
				<img style="float:right; margin-top:26px; margin-right:15px;" height="30" src="https://crystaltechify.com/images/crystaltechifydarkgrey.png">
				<h1>Create your Account</h1>
				<div class="body">
					<form id="crystaltechify-form" action="<?php echo htmlspecialchars( admin_url( 'admin-post.php' ) ); ?>" method="post">
						...
						<p>Your email address and phone number will be used to communicate important account information and to
							provide a better support experience for any enquiries with your crystaltechify account.</p>

						<?php wp_nonce_field( 'crystaltechify-create' ); ?>
						<input type="hidden" name="action" value="crystaltechify_create"/>
			...
			?>
			<div id="crystaltechify-container">
	...
	*/
	public function ebay_tab() {
		$adminUrl = admin_url( 'crystaltechify/ebaytab/0/'.get_option( 'crystaltechify_merchantid' ).'/' );

		$this->admin_tab( $adminUrl, 'crystaltechify-bulk-editor' );
	}
	...
	*/
	public function listings() {
		$adminUrl = admin_url( 'crystaltechify/ebaytab/0/'.get_option( 'crystaltechify_merchantid' ).'/listings/' );
	...
	*/
	public function analytics() {
		$adminUrl = admin_url( 'crystaltechify/ebaytab/0/'.get_option( 'crystaltechify_merchantid' ).'/analytics/' );
	...
	*/
	public function orders() {
		$adminUrl = admin_url( 'crystaltechify/ebaytab/0/'.get_option( 'crystaltechify_merchantid' ).'/orders/' );
	...
	*/
	public function account() {
		$adminUrl = admin_url( 'crystaltechify/ebaytab/0/'.get_option( 'crystaltechify_merchantid' ).'/account/' );

		$this->admin_tab( $adminUrl, 'crystaltechify-account' );
	}
	...
	*/
	public function settings() {
		...
		$adminUrl = admin_url( 'crystaltechify/settings/' );

		$this->admin_tab( $adminUrl, 'crystaltechify-settings' );
	}
	...
	*/
	...
	/**
	* admin_menu hook handler used to add the crystaltechify menu entries to the
	...
	*/
	public function admin_body_class( $classes ) {
		if ( isset($_GET['page'] ) ) {
			...
			if ( substr( $page, 0, 7 ) === 'crystaltechify' ) {
				if ( $page === 'crystaltechify' ) {
					return "$classes crystaltechify";
				} elseif ( $page === 'crystaltechify-templates' ) {
					return "$classes $page";
				} elseif ( $page === 'crystaltechify-multisite' ) {
					return "$classes $page";
				}

				return "$classes crystaltechify $page";
			}
		}
	...
	/**
	* admin_scripts hook used to apply the crystaltechify admin css+js
	*
	* @param string $hook the top level plugin page
	*/
	public function admin_scripts( $hook ) {

		if ( preg_match ( '/crystaltechify(?:-orders|-categories|-attributes|-import|-templates|-settings|-account|-listings|-analytics|)$/', $hook ) ) {

			wp_enqueue_style( 'crystaltechify-style' );
			wp_enqueue_script( 'crystaltechify-script' );
	...
	* used to notify bulk changes to products to crystaltechify
	*
	...
	* used to notify changes to currency setting to crystaltechify
	*
	...
	*/
	...
	/**
	* save_post hook handler used to notify changes to products to crystaltechify
	*
	...
	*/
	...
	/**
	* takes collected set of signals during post handling and transmits to crystaltechify
	*
	...
	public function signal_edits() {
		...
		if ( is_array( $this->ping ) &&
			...
			$response = wp_remote_post(
				'https://api.crystaltechify.com/'.get_option( 'crystaltechify_merchantid' ),
				array(
					'method'		=> 'POST',
					'timeout'		=> 5,
					'redirection' => 0,
					'httpversion' => '1.0',
					'blocking'	=> true,
					'headers'		=> array( 'X-HostKey' => get_option( 'crystaltechify_key' ) , 'Content-Type' => 'application/x-www-form-urlencoded' ),
	...
	*/
	public function site_verification() {
		...
		$site_verification = get_option('crystaltechify_site_verification');
	...
	*/
	public function conversion_tracking() {
		...
		$conversion_tracking = get_option('crystaltechify_conversion_tracking');
		...
		if( $conversion_tracking
			...
			wp_enqueue_script( 'crystaltechify-conversion-tracking' , $conversion_tracking_url, array() , $conversion_tracking );
		}
	...
	*/
	public function conversion_emit( $order_id ) {
		...
		$conversiondata = 'window.crystaltechifyConversion = { transaction_id : '.$order_id.', value : '.($order->get_total() ? $order->get_total() : 0).', currency : "'.get_woocommerce_currency().'"};';

		wp_add_inline_script( 'crystaltechify-conversion-tracking', $conversiondata );
	...
	*/
	public function add_ebay_product_tab( $tabs ) {
		...
		$tabs['crystaltechify'] = array(
								'label'	=> __( 'Channel Cloud', 'crystaltechify-linq' ),
								'target' => 'crystaltechify_product_data',
								'class'	=> '',
							);
	...
	*/
	public function ebay_product_tab_content() {
		...
		?>
			<div id="crystaltechify_product_data" class="panel woocommerce_options_panel" style="padding: 8px;">
			<iframe id="crystaltechify-control-panel" style="width: 100%;" src="<?php echo htmlspecialchars( admin_url( '/crystaltechify/ebaytab/product/'. $post->ID ).'/' ); ?>" frameborder="0"></iframe>
			</div>
		<?php
	}
	...
	*/
	public function plugin_links( $links ) {
		...
		$action_links = array(
			'listings' => '<a href="' . admin_url( 'admin.php?page=crystaltechify' ) . '" title="'.esc_html__( 'Manage Google, Amazon, eBay & Walmart Listings', 'crystaltechify-linq' ).'">'.esc_html__( 'Manage Google, Amazon, eBay & Walmart Listings', 'crystaltechify-linq' ).'</a>',
			'settings' => '<a href="' . admin_url( 'admin.php?page=crystaltechify-settings' ) . '" title="'.esc_html__( 'crystaltechify Settings', 'crystaltechify-linq' ).'">'.esc_html__( 'Settings', 'crystaltechify-linq' ).'</a>'
		);
	...
	*/
	function admin_notice_info() {
		...
		if ( get_transient( 'crystaltechify-admin-notice' ) ){
			$class = 'notice notice-info is-dismissible';
			printf( '<div class="%1$s"><p>'.esc_html__( 'crystaltechify LINQ Successfully Activated!', 'crystaltechify-linq' ).' '.
			wp_kses(
				__('<a class="button action" href="admin.php?page=crystaltechify">Click here</a> to get started.' ),
				array(
					'a' => array(
						'class' => array(),
						'href' => array()
	...
	*/
	public function init_plugin() {
		...
		add_rewrite_rule(
			'^'.preg_quote( ltrim( $syncUrl, '/' ), '/' ).'crystaltechify-sync\/(.*)?',
			'index.php?crystaltechify=sync&crystaltechify-sync-route=$matches[1]',
			'top' );
		...
		// proxy end point
		add_rewrite_rule(
			'^'.preg_quote( ltrim( $adminUrl, '/'), '/').'crystaltechify\/(.*)?',
			'index.php?crystaltechify=proxy&crystaltechify-proxy-route=$matches[1]',
			'top'
		);

		wp_register_style( 'crystaltechify-style', plugins_url( 'styles.css', __FILE__ ) );
		wp_register_script( 'crystaltechify-script', plugins_url( 'admin.js', __FILE__ ) );

		add_filter( 'query_vars', 							array( $this, 'query_vars' ) );
		add_filter( 'nocache_headers',						array( $this, 'nocache_headers' ) );
		add_action( 'parse_request',						array( $this, 'parse' ), 0 );
		add_action( 'admin_post_crystaltechify_create',			array( $this, 'create_account' ) );
		add_action( 'admin_post_crystaltechify_update_template',	array( $this, 'update_template' ) );
	...
	*/
	public static function init() {
		...
		if ( is_null( self::$_instance ) ) {
			...
			register_activation_hook( __FILE__, array( 'crystaltechifyConnect', 'activate' ) );
			add_action( 'init', array( self::$_instance, 'init_plugin' ) );

			if ( preg_match( '/\/crystaltechify-sync\//', $_SERVER['REQUEST_URI'] ) ) {
	...
	*/
	public static function activate() {
		...
		set_transient( 'crystaltechify-admin-notice', true, 20 );
